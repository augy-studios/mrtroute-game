// Service worker for MRT Navigator Game.
//
...
//   /api/                   not intercepted: questions need the network
...
async function fillShell() {
  ...
  if (failed.length) {
    console.warn(
      `[mrtnav] ${failed.length} of ${PRECACHE.length} precache entries failed. ` +
...
self.addEventListener("fetch", (event) => {
  ...
  // Questions and the leaderboard: network only, and the page says so offline.
